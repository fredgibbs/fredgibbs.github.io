---
layout: default
title: tutorials | fredgibbs  
---

# Tutorials

## QGIS
- [Making a Map](qgis/making-a-map-with-qgis.html)
- [Linking and Stlying Data](qgis/linking-and-styling-data-with-qgis.html)
- [Overlaying Historic Maps](qgis/overlaying-historic-maps-with-qgis.html)


## Data Work
- [Extract, Transform, and Save a CSV file with Python](extract-transform-save-csv.html)
- [Extract and Geocode Placenames from a Text File with Python](extract-geocode-placenames-from-text-file.html)
- [Create a KML file with Python](create-kml-file-python.html)


## New Media
- [Creating a slideshow voiceover](slideshow-voice-over.html)


## Text Mining
- [Detecting Document Similarity with R](document-similarity-with-r.md)